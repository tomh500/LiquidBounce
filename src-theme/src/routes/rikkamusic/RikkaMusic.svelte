<script lang="ts">
    ...
    import {
        controlRikkaMusic, deleteScreen, getRikkaMusicLibrary, getRikkaMusicLoginStatus,
        getRikkaMusicPlaylist, getRikkaMusicQrCodeUrl, getRikkaMusicSettings, getRikkaMusicState,
        searchRikkaMusic, startRikkaMusicLogin, updateRikkaMusicSetting,
        type RikkaMusicLibrary, type RikkaMusicLoginStatus, type RikkaMusicPlaylist,
        type RikkaMusicSetting, type RikkaMusicSong, type RikkaMusicState
    } from "../../integration/rest";

    type View = "liked" | "favorites" | "cloud" | "local" | "search" | "settings";
    ...
    let state: RikkaMusicState = {playing: false, progress: 0, volume: 70, song: null, theme: "LiquidBounce"};
    let view: View = "liked";
    let search = "";
    ...
    $: songs = view === "search" ? searchResults : selected?.songs ?? [];
    ...
    onMount(() => {
        ...
        return () => { window.clearInterval(stateTimer); window.clearInterval(loginTimer); };
    });

    async function initialize() { await Promise.all([refreshLibrary(), refreshState()]); }
    ...
    async function runSearch() {
        ...
        try { view = "search"; searchResults = await searchRikkaMusic(search.trim()); error = ""; }
        catch (e) { error = e instanceof Error ? e.message : "搜索失败"; }
    }
    async function playPlaylist(index: number) {
        try {
            if (view === "search") await controlRikkaMusic("play-search", undefined, undefined, index, search);
    ...
    async function saveSetting(setting: RikkaMusicSetting, value?: string | number | boolean) {
        try { await updateRikkaMusicSetting(setting.key, value ?? setting.value); await loadSettings(); await refreshState(); }
...
<div class="rikkamusic-stage">
    <div class:light={state.theme === "Light"} class="rikkamusic-shell">
        ...
        <div class="main-body">
            <aside class="sidebar"><div class="menu-group"><div class="menu-title">我的</div><button class:active={view === "liked" && selected?.id === library?.liked?.id} class="menu-item" on:click={() => library?.liked && openPlaylist(library.liked)}><span>♥</span>我喜欢的音乐</button><button class:active={view === "favorites"} class="menu-item" on:click={() => view = "favorites"}><span>★</span>我的收藏</button><button class:active={view === "cloud"} class="menu-item" on:click={() => view = "cloud"}><span>☁</span>我的音乐网盘</button><button class:active={view === "local"} class="menu-item" on:click={() => view = "local"}><span>▣</span>本地挂载歌单</button></div>{#if (library?.playlists?.length ?? 0) > 0}<div class="menu-group playlist-group"><div class="menu-title">创建的歌单</div>{#each library?.playlists ?? [] as playlist (playlist.id)}<button class:active={selected?.id === playlist.id && view === "liked"} class="menu-item playlist-item" on:click={() => openPlaylist(playlist)}><span>♫</span>{playlist.name}</button>{/each}</div>{/if}</aside>
            <main class="content-area">
                ...
                {:else if view === "favorites"}<section class="view-page"><h2>我的收藏</h2><div class="sub-tabs"><span class="tab active">专辑</span><span class="tab">歌手</span><span class="tab">歌单</span></div><p class="placeholder-text">暂无收藏内容</p></section>
                {:else if view === "cloud"}<section class="view-page"><h2>我的音乐网盘</h2><div class="cloud-bar"><span>云盘空间数据将在网易云接口提供后显示</span><button class="btn btn-secondary" disabled>上传音乐</button></div><p class="placeholder-text">网盘暂无数据</p></section>
                {:else if view === "local"}<section class="view-page"><h2>挂载本地音乐歌单</h2><div class="mount-panel"><div class="mount-options"><span>本地挂载目录</span><input aria-label="本地挂载目录" value="未配置" readonly /><button class="btn btn-secondary" disabled>选择本地文件夹</button></div><p class="mount-tip">本地文件读取尚未接入播放器，已保留原版界面入口和列表位置。</p></div><div class="mount-result"><div class="result-header"><h3>本地解析出的歌曲列表 (0)</h3><button class="btn btn-secondary" disabled>播放本地全部</button></div><div class="empty-cell">未挂载文件夹或未读取到音频文件</div></div></section>
                {:else if view === "settings"}<section class="settings-page"><h2>设置</h2><div class="setting-tabs">{#each Object.entries(categoryLabels) as [category, label]}<button class:active={settingsCategory === category} on:click={() => selectSettingsCategory(category as SettingsCategory)}>{label}</button>{/each}</div>{#if settingsLoading}<div class="placeholder loading"><span class="spinner"></span>正在读取设置</div>{:else}<div class="setting-group"><h3>{categoryLabels[settingsCategory]}设置</h3>{#if settingsCategory === "account"}<p class="settings-note">账号 Cookie 不会显示在网页中。请通过右上角的扫码登录更新账号。</p>{/if}{#each settings as setting (setting.key)}<div class="setting-item"><span class="setting-label">{labelFor(setting)}{#if setting.hotkey}<small>快捷键 {setting.hotkey}</small>{/if}</span>{#if setting.type === "boolean"}<input aria-label={labelFor(setting)} class="switch" type="checkbox" checked={Boolean(setting.value)} on:change={(event) => saveSetting(setting, event.currentTarget.checked)} />{:else if setting.type === "option"}<button class="select-like" on:click={() => saveSetting(setting)}>{setting.value}<span>⌄</span></button>{:else if (setting.type === "integer" || setting.type === "double") && setting.min !== undefined && setting.max !== undefined}<div class="range-control"><input aria-label={labelFor(setting)} type="range" min={setting.min} max={setting.max} step={setting.type === "double" ? "0.01" : "1"} value={setting.value} on:change={(event) => saveSetting(setting, Number(event.currentTarget.value))}/><output>{setting.value}</output></div>{:else if setting.type === "color"}<div class="color-control"><input aria-label={labelFor(setting)} type="color" value={String(setting.value).slice(0, 7)} on:change={(event) => saveSetting(setting, event.currentTarget.value.replace("#", "#FF"))}/><input aria-label={`${labelFor(setting)} 色值`} value={setting.value} on:change={(event) => saveSetting(setting, event.currentTarget.value)} /></div>{:else}<input aria-label={labelFor(setting)} class="value-input" value={setting.value} on:change={(event) => saveSetting(setting, event.currentTarget.value)} />{/if}</div>{:else}<p class="placeholder-text">此分类没有可配置项</p>{/each}</div>{/if}</section>
                {:else}<section class="view-page"><div class="playlist-header"><div class="cover-art"><img src={selected?.cover || ""} alt="" /><span>♥</span></div><div class="playlist-info"><h2>{title}</h2><p class="meta">{library.username} 创建 · {songs.length} 首歌曲</p><button class="btn btn-primary" disabled={!songs.length} on:click={() => playPlaylist(0)}>▶ 播放全部</button></div></div>{#if view === "search"}<h2 class="search-title">搜索 “{search}”</h2><div class="sub-tabs"><span class="tab active">综合</span><span class="tab">单曲</span><span class="tab">歌单</span><span class="tab">歌手</span></div>{/if}<div class="song-table"><div class="song-row song-head"><span>#</span><span>标题</span><span>歌手</span><span>专辑</span><span>时长</span></div>{#each songs as song, index (song.id)}<button class:playing={state.song?.id === song.id} class="song-row" on:click={() => playPlaylist(index)}><span>{state.song?.id === song.id ? "▶" : index + 1}</span><span class="song-title">{song.name}</span><span>{song.artist || "未知歌手"}</span><span>{song.album || "未知专辑"}</span><span>{formatTime(song.duration)}</span></button>{:else}<div class="empty-cell">暂无歌曲</div>{/each}</div></section>{/if}
            </main>
        </div>

        <footer class="player-bar"><div class="track-info"><div class="small-cover"><img src={state.song?.cover || ""} alt="" /></div><div class="track-text"><strong>{state.song?.name ?? "未选择播放歌曲"}</strong><small>{state.song?.artist ?? "--"}</small></div></div><div class="player-center"><div class="player-controls"><button title="随机播放" aria-label="随机播放">⌘</button><button on:click={() => control("previous")} title="上一首" aria-label="上一首">◀◀</button><button class="play-btn" on:click={() => control("toggle")} aria-label="播放或暂停">{state.playing ? "Ⅱ" : "▶"}</button><button on:click={() => control("next")} title="下一首" aria-label="下一首">▶▶</button><button title="歌词" aria-label="歌词">♬</button></div><div class="progress-container"><span>{formatTime(state.progress)}</span><input type="range" min="0" max={state.song?.duration ?? 1} value={state.progress} on:change={(event) => control("seek", Number(event.currentTarget.value))} /><span>{formatTime(state.song?.duration ?? 0)}</span></div></div><div class="extra-controls"><span class="quality-tag">无损</span><div class="volume-container"><span>🔊</span><input type="range" min="0" max="100" value={state.volume} on:change={(event) => control("volume", Number(event.currentTarget.value))} /></div></div></footer>
